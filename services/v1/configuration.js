    ...
    wishlistButton: async(requestData,requestParam) => {     
        try {
        		let accessToken = await authService.getShopifyToken();
 				...
 				//add snipet to the theme
 				let snipet = '{% if customer.id != null %}<button class="wishlist-btn btn btn--small" prod-id="{{ product.id }}" customer-id="{{ customer.id }}">Wishlist</button>{% endif %}<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script><script type="text/javascript" src="https://a1a2b3df7c9f.ngrok.io/assets/js/wishlist-app.js"></script>'
 				let snipetPayload =  {
